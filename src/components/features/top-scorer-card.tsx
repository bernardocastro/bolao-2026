...
export function TopScorerCard({ poolId }: TopScorerCardProps) {
  ...
  const { mutate: savePick, isPending: isSaving } = useMutation({
    mutationFn: () =>
      api('/api/top-scorer', { method: 'POST', json: { poolId, playerName: selected } }),
    onSuccess: () => {
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        {closed && !result && <Lock className="ml-auto h-4 w-4 text-muted-foreground" />}
      </div>
      ...
      {result ? (
        <div className="space-y-2">
          ...
          {pick && (
            ...
            >
              ...
              {isCorrect ? (
                <span>
                  Você acertou! <strong>+{bonus} pts</strong>
                </span>
              ) : (
                <span>
                  Seu palpite: <strong>{pick}</strong>
                </span>
              )}
            </div>
          )}
          ...
          {isWrong && <p className="text-xs text-muted-foreground">Não foi dessa vez. 😅</p>}
        </div>
      ) : closed ? (
        <div className="space-y-2">
        ...
        <div className="space-y-3">
          ...
          <PlayerCombobox
            ...
            placeholder="Escolha o artilheiro..."
          />
          <Button
            size="sm"
